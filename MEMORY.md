...
| 2026-09-17 | Button "Alle ausbezahlen" → "Alle als Ausbezahlt markieren" | Zahlstapel-Button in `PaystackView.vue` (i18n-Key `payAll`) umbenannt, um klarzustellen, dass nur der Status gesetzt wird. DE: "Alle als Ausbezahlt markieren", EN: "Mark all as Paid". Build + Deploy erfolgt (nextcloud3, App `spesenerfassung`). |
| 2026-09-17 | Bild-Belege beim Upload verkleinern | Bilder (JPG/PNG) werden vor dem Speichern auf längste Kante 1600 px reduziert (JPEG Q80, PNG bleibt PNG). Frontend `src/imageResize.js` via Canvas (spart Bandbreite, umgeht `upload_max_filesize`); Backend `ReceiptService::resizeImage()` als Sicherheitsnetz via GD (Fallback ohne GD: Original). Gespeichert wird nur das reduzierte Bild. **Auf nextcloud3 deployed als App `spesenerfassung` 1.0.10** (GD/Imagick vorhanden, Resize getestet: 3000×2000 → 1600×1067). |
| 2026-09-17 | Spesenbeleg-Link in Auswertung | `EvaluationView.vue` neue Spalte "Spesenbeleg" mit PDF-Download-Link (nur wenn `bookingReceipt`-Flag=true). `ApprovalController::bookingReceipt(int)` GET-Endpoint (`/api/approvals/booking-receipt/{id}`, NoCSRFRequired, nur Präsident/Kassier) liest PDF aus kassier-Ordner via `BookingReceiptService::exists()/getFile()`. `evaluation()` setzt Flag; CSV-Export erhält Spalte "Spesenbeleg" (Ja/Nein). Build erneuert. |
| 2026-09-17 | Spesenbeleg-PDF Vorschau (analog Belege) | `bookingReceiptPreview(int)` GET-Endpoint (`/api/approvals/booking-receipt/{id}/preview`, DataDisplayResponse=inline) fuer Browser-Vorschau. `EvaluationView` Spesenbeleg-Zelle spiegelt Beleg-Muster: Hover-Popover + "Vorschau oeffnen"-Link (target=_blank). Sichtbarkeit weiter ueber `bookingReceipt`-Flag (Datei existiert ab Status paid/done). Build erneuert + auf nextcloud3 deployed. |
| 2026-09-17 | Fix: Spesenbeleg-Anhänge fehlten (113/114/115) | Ursache: alte `ReceiptService::validateFile()` gab Fehlertexte statt `null` zurück; `upload()` speicherte die Datei trotzdem und schrieb den Text als `mime_type` in die DB (`File too large…` / `Invalid file type…`). `BookingReceiptService` verzweigt exakt auf `application/pdf`/`image/*` und übersprang die Anhänge still. Fix: MIME per `finfo::buffer` aus Inhalt, sichtbarer Hinweis + Log statt Skip, Bild-Downscale (max 1600 px/Q82) vor dem Einbetten. Ablageort auf `treasurer_uid` vereinheitlicht (Schreiben=Lesen); `Makerspace` ist im Kassier-Home ein Mount auf `home::admin`, daher physisch derselbe Ordner. Neuer occ-Befehl `spesenerfassung:regenerate-booking-receipt <id…>`; Version 1.0.11. Verifiziert: 110/114/115 je 2 Seiten mit eingebettetem Anhang. |
| 2026-09-17 | Dateiablage: Belege vs. Spesenbeleg | Belege (PDF/JPG/PNG) über `IAppData`: DB `oc_<prefix>sp_receipts.file_path = receipts/<expenseId>/<datei>`, physisch `<datadirectory>/appdata_<instanceid>/spesenerfassung/receipts/<expenseId>/<datei>` (kein Zugriff über Datei-UI). Spesenbeleg-PDF im `booking_folder` (z.B. `Makerspace/Vorstand/Spesen`) des `treasurer_uid`; physisch bei admin, weil `Makerspace` ein Mount auf `home::admin` ist. Neu erzeugen: `occ spesenerfassung:regenerate-booking-receipt <id…>` (Zielordner muss existieren). |
| 2026-09-17 | Fix: DataDisplayResponse-Signatur | `DataDisplayResponse` erwartet `(content, int statusCode, array headers)` — KEIN Dateinamen-/Content-Type-Argument. Content-Type muss via Header gesetzt werden (siehe `ExpenseController::previewReceipt`): `'Content-Type' => 'application/pdf'`, `'X-Content-Type-Options' => 'nosniff'`, `'Content-Security-Policy' => "default-src 'none'; sandbox"`. Falscher Aufruf mit 3. Arg `'application/pdf'` fuehrte zu 500 (Argument #2 int, string). |
...
- [ ] Zweite Installation (1.0.11): `Spesenbeleg_25.pdf`/`Spesenbeleg_28.pdf` neu erzeugen — `occ spesenerfassung:regenerate-booking-receipt 25 28` (Belege fehlen dort ohne Anhang)
